...
export default function HeroSection({ children }) {
  ...
  // Scroll-triggered animation with better performance and max scale limit
  const { scrollYProgress } = useScroll({
    target: macbookRef,
    offset: ["start end", "end start"]
  })
  
  // Smooth scale transform with spring physics and maximum scale limit
  const scaleRaw = useTransform(scrollYProgress, [0.2, 0.6], [1, 1.15])
  ...
  return (
    <div className="grainy-bg min-h-screen overflow-x-hidden relative">
      ...
      {/* MacBook Mockup with smooth hardware-accelerated animation */}
      <motion.div
        ref={macbookRef}
        ...
        className="relative px-4 max-w-7xl mx-auto smooth-transform"
      >
        <div className="max-w-6xl mx-auto">
          {/* MacBook Frame */}
          <div className="relative smooth-transform">
            {/* Screen with enhanced drop shadow */}
            <div className="bg-gray-900 rounded-t-[20px] p-4 shadow-2xl drop-shadow-2xl" 
                 style={{ 
                   filter: 'drop-shadow(0 25px 50px rgba(0, 0, 0, 0.25)) drop-shadow(0 10px 25px rgba(0, 0, 0, 0.15))',
            ...
            >
              ...
              {/* Screen Content - Survey is always visible and interactive */}
              <div 
                className="bg-white rounded-lg overflow-hidden h-[600px]" 
                id="survey-container"
              >
                {/* Survey content rendered directly */}
            ...
            {/* MacBook base with shadow */}
            <div className="bg-gray-300 h-6 rounded-b-[20px] shadow-lg"></div>
            <div className="bg-gray-400 h-3 w-40 mx-auto rounded-b-xl shadow-md"></div>
          </div>
        </div>
      </motion.div>

      {/* Extra Spacer before Features */}
